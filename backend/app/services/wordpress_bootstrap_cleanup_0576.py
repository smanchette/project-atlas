"""Strict cleanup profile for upgrade bootstrap 0.2.0 after bridge 0.57.6 verification."""
...
from app.services.wordpress_plugin_upgrade_0576 import (
...
DEACTIVATION_PHRASE = "DEACTIVATE PROJECT ATLAS UPGRADE BOOTSTRAP 0.2.0"
DELETION_PHRASE = "DELETE PROJECT ATLAS UPGRADE BOOTSTRAP 0.2.0"
...
def _common_gates(request, installation, activation, upgrade, installations, activations, upgrades, cleanups, lifecycle, metadata_states, metadata_audits, observed, plugin_status, bootstrap_status, bootstrap_artifact, evidence_valid, evidence_reason):
    ...
    return [
        ...
        _gate("upgrade_audit", "Selected 0.57.5 to 0.57.6 upgrade audit is verified", bool(upgrade and upgrade.id == request.upgrade_audit_id and upgrade.status == "verified" and upgrade.previous_version == "0.57.5" and upgrade.target_version == TARGET_VERSION and upgrade.recovery_recommendation == "no_action"), "Verified 0.57.5 to 0.57.6 upgrade audit required."),
        ...
        _gate("bridge_identity", "Metadata Bridge 0.57.6 identity is exact", len(matches) == 1 and matches[0].get("version") == request.expected_bridge_version == TARGET_VERSION and request.expected_bridge_slug == PLUGIN_SLUG and request.expected_bridge_path == PLUGIN_FILE and _normalize_plugin_identifier(matches[0].get("plugin")).authorized_entry_path == PLUGIN_FILE, "Metadata Bridge identity differs."),
        ...
        _gate("bridge_checksum", "Metadata Bridge executable matches 0.57.6", plugin_status.get("checksum") == artifact.get("plugin_source_sha256") and request.expected_bridge_zip_sha256 == artifact.get("zip_sha256"), "Metadata Bridge artifact differs."),
