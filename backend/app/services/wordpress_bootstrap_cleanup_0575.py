"""Historical, immutable bootstrap 0.1.0 cleanup profile."""
...
from app.services.wordpress_plugin_upgrade_0575 import (
...
DEACTIVATION_PHRASE = "DEACTIVATE PROJECT ATLAS UPGRADE BOOTSTRAP"
DELETION_PHRASE = "DELETE PROJECT ATLAS UPGRADE BOOTSTRAP"
...
def _common_gates(request, installation, activation, upgrade, installations, activations, upgrades, cleanups, lifecycle, metadata_states, metadata_audits, observed, plugin_status, bootstrap_status, bootstrap_artifact, evidence_valid, evidence_reason):
    ...
    return [
        ...
        _gate("upgrade_audit", "Upgrade audit 1 is verified with no recovery required", bool(upgrade and upgrade.id == 1 and upgrade.status == "verified" and upgrade.target_version == TARGET_VERSION and upgrade.recovery_recommendation == "no_action"), "Verified upgrade audit 1 with no_action recovery required."),
        ...
        _gate("bridge_identity", "Metadata Bridge 0.57.5 identity is exact", len(matches) == 1 and matches[0].get("version") == request.expected_bridge_version == TARGET_VERSION and request.expected_bridge_slug == PLUGIN_SLUG and request.expected_bridge_path == PLUGIN_FILE and _normalize_plugin_identifier(matches[0].get("plugin")).authorized_entry_path == PLUGIN_FILE, "Metadata Bridge identity differs."),
        ...
        _gate("bridge_checksum", "Metadata Bridge executable matches 0.57.5", plugin_status.get("checksum") == artifact.get("plugin_source_sha256") and request.expected_bridge_zip_sha256 == artifact.get("zip_sha256"), "Metadata Bridge artifact differs."),
        ...
        _gate("metadata_rows", "Atlas metadata state and sync-audit rows remain zero", len(metadata_states) == len(metadata_audits) == 0, "Metadata rows exist."),
        ...
        _gate("payload_absent", "Metadata payload and hash remain empty", snapshot.get("payload") is None and not snapshot.get("payload_hash"), "Metadata payload exists."),
...
def _preservation_gates(request, before, after, plugin_status, expected):
    ...
    return [
        ...
        _gate("payload_absent", "Payload and hash remain empty", snapshot.get("payload") is None and not snapshot.get("payload_hash"), "Metadata payload changed."),
        _gate("revision_zero", "Metadata revision remains zero", str(snapshot.get("revision")) == "0", "Metadata revision changed."),
