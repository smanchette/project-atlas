...
from app.services import wordpress_bootstrap_cleanup_0577 as cleanup
...
def seed(session):
    ...
    session.add(WordPressPluginUpgradeAudit(
        id=3, generated_page_id=41, wordpress_post_id=8, installation_audit_id=1, activation_audit_id=1,
        status="verified", operator="Shawn Manchette", confirmation_phrase_hash="d" * 64, handle_fingerprint="e" * 64,
        binding_hash="f" * 64, previous_version="0.57.6", target_version="0.57.7",
        previous_artifact_sha256=upgrade.CURRENT_ZIP_SHA256, target_artifact_sha256=artifact()["zip_sha256"],
        release_identity={}, backup_evidence={}, browser_evidence_id="upgrade-0577", browser_evidence_hashes={},
...
def test_cleanup_020_deactivation_and_deletion_are_separately_phrase_gated(monkeypatch, db):
    ...
    with Session(db) as session:
        ...
        assert preflight.confirmation_phrase == "DEACTIVATE PROJECT ATLAS UPGRADE BOOTSTRAP 0.3.0"
        ...
        assert deletion.confirmation_phrase == "DELETE PROJECT ATLAS UPGRADE BOOTSTRAP 0.3.0"
