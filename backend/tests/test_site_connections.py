...
@pytest.mark.parametrize(
    "model",
    (NavigationSet, NavigationItem, InternalLinkIntent),
)
def test_navigation_decision_schema_preserves_timezone_awareness(model):
...
def test_backup_057_rejects_incomplete_current_connection_snapshot(
    ...
    with Session(engine) as session:
        ...
        create_initial_composition_revision(
            session,
            composition,
            recorded_by="test:site-connections",
