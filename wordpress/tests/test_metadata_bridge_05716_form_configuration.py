...
def test_05716_private_admin_surface_is_capability_nonce_post_and_allowlist_bound() -> None:
...
def test_05716_admin_page8_email_action_is_server_built_cas_and_route_bound() -> None:
    source = DELIVERY.read_text(encoding="utf-8")
    allowlist = _function(
        source, "atlas_performance_local_v5_form_delivery_admin_allowed_keys"
    )
...
def test_05716_page_payload_private_scan_preserves_only_exact_governed_public_contact() -> None:
    ...
    assert "array_is_list($sections)" in final_index
    assert "($section['key'] ?? null) !== 'final_conversion'" in final_index
    assert "if ($match !== null) { return null; }" in final_index
    assert "$candidate_final_index === $prior_final_index" in private_scan
    assert "$candidate_section === $prior_section" in private_scan
    assert "$candidate_body === $prior_body" in private_scan
    assert "$recipient_scan['sections'][$candidate_final_index]['body'] = null" in private_scan
...
def test_05716_configuration_is_server_built_disabled_first_and_exactly_one_option() -> None:
    source = DELIVERY.read_text(encoding="utf-8")
    target = _function(
...
def test_05716_governed_optional_email_and_reply_to_contract_is_exact() -> None:
    source = DELIVERY.read_text(encoding="utf-8")
    field = _function(
...
def test_05716_canonical_domain_exception_is_exact_and_staging_only() -> None:
    source = DELIVERY.read_text(encoding="utf-8")
    canonical = _function(
...
def test_05716_saved_status_and_admin_html_are_redacted_and_replacement_inputs_blank() -> None:
    source = DELIVERY.read_text(encoding="utf-8")
    status = _function(
...
def test_05716_enable_requires_current_exact_payload_and_disable_preserves_configuration() -> None:
    source = DELIVERY.read_text(encoding="utf-8")
    enable = _function(
...
def test_05716_public_surfaces_still_exclude_delivery_configuration() -> None:
    source = DELIVERY.read_text(encoding="utf-8")
