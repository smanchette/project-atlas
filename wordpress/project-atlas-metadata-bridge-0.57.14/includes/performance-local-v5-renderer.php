...
function atlas_performance_local_v5_asset_path($value): bool {
    ...
    return preg_match(
        '#^/wp-content/uploads/(?:atlas-v5/[A-Za-z0-9][A-Za-z0-9._/-]*|[1-9][0-9]{3}/(?:0[1-9]|1[0-2])/[A-Za-z0-9][A-Za-z0-9._-]*)\.(?:avif|jpe?g|png|svg|webp)$#i',
        $value
    ) === 1;
}
...
function atlas_performance_local_v5_runtime_files(): array {
    return [
        ...
        ATLAS_PERFORMANCE_LOCAL_V5_FORM_DELIVERY_MODULE,
    ];
}
        ...
        <div class="performanceLocalContainer performanceLocalFinalGrid">
            <div class="performanceLocalSectionCopy">
                ...
                <div class="performanceLocalActionRow">
                    ...
                    <?php if ($website['contact_email'] !== null): ?><a class="performanceLocalButton performanceLocalButtonSecondary" href="<?php echo esc_attr('mailto:' . $website['contact_email']); ?>"><?php echo esc_html($website['contact_email']); ?></a><?php endif; ?>
                </div>
            </div>
...
function atlas_performance_local_v5_render_form(array $form, bool $compact = false): void {
    ...
    $delivery = function_exists('atlas_performance_local_v5_form_delivery_render_context')
        ? atlas_performance_local_v5_form_delivery_render_context($form)
        : null;
    $active = is_array($delivery);
    $autocomplete_by_key = [
        'name' => 'name',
        'phone' => 'tel',
    ...
    <form
        ...
        aria-label="<?php echo esc_attr($active ? 'Estimate request form' : 'Estimate request preview'); ?>"
        autocomplete="<?php echo $active ? 'on' : 'off'; ?>"
        <?php if ($active): ?>
            novalidate
            data-atlas-v5-active-form
            data-atlas-v5-form-endpoint="<?php echo esc_url($delivery['endpoint']); ?>"
            action="<?php echo esc_url($delivery['endpoint']); ?>"
            method="post"
            data-preview-only="false"
            data-provider-state="wordpress_native"
            data-provider-configured="true"
            data-collects-data="true"
            data-controls-read-only="false"
        <?php else: ?>
    ...
    >
        <?php if (!$active): ?><p class="performanceLocalV5FormNotice performanceLocalFormNotice" role="note"><?php echo esc_html($form['notice']); ?></p><?php endif; ?>
        <div class="performanceLocalV5FormGrid">
            <?php foreach ($form['fields'] as $field):
                $autocomplete = $autocomplete_by_key[$field['field_key']] ?? null;
                $control_id = $form['anchor'] . '-' . $field['field_key'];
                $error_id = $control_id . '-error';
            ?>
                <label for="<?php echo esc_attr($control_id); ?>"<?php echo $field['control'] === 'textarea' ? ' class="performanceLocalV5FormFieldFull"' : ''; ?>>
                    <span><?php echo esc_html($field['label']); ?></span>
                    <?php if ($field['control'] === 'textarea'): ?>
                        <textarea
                            id="<?php echo esc_attr($control_id); ?>"
                            <?php if ($active): ?>name="<?php echo esc_attr($field['field_key']); ?>"<?php endif; ?>
                            ...
                            data-validation-rule="<?php echo esc_attr($field['validation']['rule']); ?>"
                            data-validation-minimum-length="<?php echo esc_attr((string) $field['validation']['minimum_length']); ?>"
                            data-validation-maximum-length="<?php echo esc_attr((string) $field['validation']['maximum_length']); ?>"
                            minlength="<?php echo esc_attr((string) $field['validation']['minimum_length']); ?>"
                            maxlength="<?php echo esc_attr((string) $field['maximum_length']); ?>"
                            rows="3"
                            <?php if ($active && $autocomplete !== null): ?>autocomplete="<?php echo esc_attr($autocomplete); ?>"<?php endif; ?>
                            <?php if (!$active): ?>readonly disabled<?php endif; ?>
                    ...
                    <?php else: ?>
                        ...
                        >
                    <?php endif; ?>
                    <span id="<?php echo esc_attr($error_id); ?>" class="performanceLocalV5FieldError" data-atlas-v5-field-error hidden>Please check this field.</span>
                </label>
            <?php endforeach; ?>
        </div>
        <?php if ($active): ?>
            <div class="performanceLocalV5FormHoneypot" aria-hidden="true">
                <label>Leave this field empty<input type="text" tabindex="-1" autocomplete="off" data-atlas-v5-honeypot></label>
            </div>
            <input type="hidden" value="<?php echo esc_attr($delivery['token']); ?>" data-atlas-v5-system="token">
            <input type="hidden" value="<?php echo esc_attr($delivery['website_identity']); ?>" data-atlas-v5-system="website_identity">
            <input type="hidden" value="<?php echo esc_attr($delivery['form_identity']); ?>" data-atlas-v5-system="form_identity">
            <input type="hidden" value="<?php echo esc_attr((string) $delivery['form_version']); ?>" data-atlas-v5-system="form_version">
            <input type="hidden" value="<?php echo esc_attr($delivery['page_identity']); ?>" data-atlas-v5-system="page_identity">
            <input type="hidden" value="<?php echo esc_attr($delivery['field_definition_hash']); ?>" data-atlas-v5-system="field_definition_hash">
            <p class="performanceLocalFormState" role="status" aria-live="polite" aria-atomic="true" tabindex="-1" data-atlas-v5-form-status hidden></p>
            <button type="submit" data-atlas-v5-form-submit disabled><?php echo esc_html($form['submit_label']); ?></button>
        <?php else: ?>
    ...
    <footer class="<?php echo esc_attr($footer_class); ?>">
        <div class="<?php echo esc_attr($container); ?>">
            ...
            <div class="<?php echo esc_attr($contact); ?>">
                ...
                <?php if ($footer['contact_email'] !== null): ?><span><?php echo esc_html($footer['contact_email']); ?></span><?php endif; ?>
            </div>
        </div>
    </footer>
    <?php
