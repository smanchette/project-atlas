 ...
 * Description: Guarded metadata bridge with a contained Performance Local V5 renderer and WordPress-native estimate delivery.
 * Version: 0.57.16
...
require_once __DIR__ . '/includes/performance-local-v5-page-payload.php';
require_once __DIR__ . '/includes/performance-local-v5-form-delivery.php';
...
function atlas_metadata_approved_public_contact_email(): ?string {
    ...
    if (!($post instanceof WP_Post)
        ...
        || $post->post_name !== 'drywood-termite-tenting-orlando-fl'
        || !is_array($payload)
        ...
        || ($payload['payload_identity']['source_composition'] ?? null) !== 'composition:41:v10'
        ...
        || ($payload['website']['company_name'] ?? null)
            !== 'Flo-Zone Pest And Termite Solutions Inc') {
        return null;
    }
    $email = $payload['website']['contact_email'] ?? null;
    return is_string($email)
        && filter_var($email, FILTER_VALIDATE_EMAIL) !== false
        && ($payload['footer']['contact_email'] ?? null) === $email
            ? $email
            : null;
}
...
function atlas_metadata_approved_payload(): array {
    ...
    $public_contact_email = atlas_metadata_approved_public_contact_email();
    if ($public_contact_email === null) { return []; }
    return ['schema_version'=>'2.0','page_id'=>41,'wordpress_post_id'=>8,'meta_description'=>$description,
        'json_ld'=>['@context'=>'https://schema.org','@graph'=>[
            ['@type'=>'Organization','@id'=>$organization_id,'name'=>$organization,'telephone'=>'(844) 600-8368','email'=>$public_contact_email,'identifier'=>['@type'=>'PropertyValue','name'=>'License','value'=>'JB360566']],
