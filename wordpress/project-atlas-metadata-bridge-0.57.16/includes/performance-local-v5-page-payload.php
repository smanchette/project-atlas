...
function atlas_performance_local_v5_page_payload_post_envelope(
    ...
    $current = atlas_performance_local_v5_page_payload_metadata_state($route_post_id);
    $prior_payload = $current['valid'] && is_array($current['value'])
        ? $current['value']
        : null;
    if (atlas_performance_local_v5_page_payload_contains_private_delivery_value(
        $body['payload'],
        $prior_payload
    )) {
        return atlas_performance_local_v5_page_payload_error(
...
function atlas_performance_local_v5_page_payload_final_conversion_index(
    array $payload
): ?int {
    $sections = $payload['sections'] ?? null;
    if (!is_array($sections) || !array_is_list($sections)) { return null; }
...
function atlas_performance_local_v5_page_payload_contains_private_delivery_value(
    array $payload,
    ?array $prior_payload = null
): bool {
    ...
    $recipient = $configuration['recipient_email'] ?? null;
    $from = $configuration['from_email'] ?? null;
    $has_recipient = is_string($recipient) && trim($recipient) !== '';
    $has_from = is_string($from) && trim($from) !== '';
    if (!$has_recipient && !$has_from) { return false; }
    if ($encoded === null) { return true; }
    if ($has_from && str_contains(strtolower($encoded), strtolower(trim($from)))) {
        return true;
    }

    if (!$has_recipient) { return false; }
    $recipient_scan = $payload;
    $prior_contact = $prior_payload['website']['contact_email'] ?? null;
    $prior_footer_contact = $prior_payload['footer']['contact_email'] ?? null;
    $candidate_contact = $payload['website']['contact_email'] ?? null;
    $candidate_footer_contact = $payload['footer']['contact_email'] ?? null;
    if (is_string($prior_contact)
        && $recipient === $prior_contact
        && $prior_footer_contact === $prior_contact
        && $candidate_contact === $prior_contact
        && $candidate_footer_contact === $prior_contact) {
        $recipient_scan['website']['contact_email'] = null;
        $recipient_scan['footer']['contact_email'] = null;

        $prior_final_index = atlas_performance_local_v5_page_payload_final_conversion_index(
            $prior_payload
        );
        $candidate_final_index = atlas_performance_local_v5_page_payload_final_conversion_index(
            $payload
        );
        if ($prior_final_index !== null
            && $candidate_final_index === $prior_final_index) {
            $prior_section = $prior_payload['sections'][$prior_final_index] ?? null;
            $candidate_section = $payload['sections'][$candidate_final_index] ?? null;
            $prior_body = is_array($prior_section) ? ($prior_section['body'] ?? null) : null;
            $candidate_body = is_array($candidate_section)
                ? ($candidate_section['body'] ?? null)
                : null;
            if (is_array($prior_section)
                && $candidate_section === $prior_section
                && is_string($prior_body)
                && is_string($candidate_body)
                && $candidate_body === $prior_body
                && str_contains(
                    strtolower($candidate_body),
    ...
    $recipient_encoded = atlas_performance_local_v5_page_payload_json($recipient_scan);
    return $recipient_encoded === null
        || str_contains(strtolower($recipient_encoded), strtolower(trim($recipient)));
}
